//
//  RegisterView.h
//  ucareshop
//
//  Created by 谢佳培 on 2019/8/28.
//  Copyright © 2019 IOSDeveloper. All rights reserved.
//

// import分组次序：Frameworks、Services、UI
#import <UIKit/UIKit.h>

//typedef void(^edithandle)(NSString *text);

#pragma mark - @class

#pragma mark - 常量

#pragma mark - 枚举

NS_ASSUME_NONNULL_BEGIN

/**
 * <#类注释，说明类的功能#>
 * @note <#额外说明的注意项，说明一些需要注意的地方，没有可取消此项。#>
 */
@interface RegisterView : UIView

@property (nonatomic, readonly) UIImageView *phoneImageView;
@property (nonatomic, readonly) UIImageView *chatImageView;
@property (nonatomic, readonly) UIImageView *passwordImageView;
@property (nonatomic, readonly) UITextField *phoneNumberTextField;
@property (nonatomic, readonly) UITextField *passwordTextField;
@property (nonatomic, readonly) UITextField *codeTextField;
@property (nonatomic, readonly) UISwitch *secretSwitch;
@property (nonatomic, readonly) UIButton *sendCodeButton;
@property (nonatomic, readonly) UIButton *conformButton;
@property (nonatomic, readonly) UIButton *agreementButton;
@property (nonatomic, readonly) UIButton *checkButton;
@property (nonatomic, readwrite) BOOL isCheck;

- (void)startTime;

@end

NS_ASSUME_NONNULL_END
