...
@property (nonatomic, strong, readwrite) UITableView *cancelOrderTable;
...
- (void)createSubViews {
    ...
    self.cancelOrderTable = [[UITableView alloc]initWithFrame:CGRectZero style:UITableViewStyleGrouped];
    self.cancelOrderTable.layer.borderWidth = 1.0;
    self.cancelOrderTable.layer.borderColor = UIColor.lightGrayColor.CGColor;
    self.cancelOrderTable.delegate = self;
    self.cancelOrderTable.dataSource = self;
    self.cancelOrderTable.backgroundColor = UIColor.whiteColor;
    self.cancelOrderTable.separatorStyle = UITableViewCellSeparatorStyleNone;
    self.refreshView = [[RefreshView alloc]initWithFrame:CGRectMake(200, -54, 30, 30)];
    [self.cancelOrderTable addSubview:self.refreshView];
    [self.cancelOrderTable registerClass:[CommodityOrderTableViewCell class] forCellReuseIdentifier:cellIdentifer];
    [self addSubview:self.cancelOrderTable];
}

// 添加约束
- (void)createSubViewsConstraints {
    [self.cancelOrderTable mas_makeConstraints:^(MASConstraintMaker *make) {
...
// 下拉刷新
- (void)setupRefresh {
    __weak CancelOrderView *weakself = self;
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        // 耗时的操作
        [weakself createData];
        dispatch_async(dispatch_get_main_queue(), ^{
            // 更新界面
            [weakself.cancelOrderTable reloadData];
...
- (void)endRefresh{
    if(self.refreshState == RefreshStateLoading){
        [UIView animateWithDuration:1.0 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            self.cancelOrderTable.contentInset = UIEdgeInsetsMake(0, 0, 0, 0);
...
- (void) startRefresh {
    self.cancelOrderTable.contentInset = UIEdgeInsetsMake(64, 0, 0, 0);//改变contentInset的值就可以取消回弹效果停留在当前位置了
...
- (void) createData {
    ...
    [tempDict setObject:@"5" forKey:@"status"];
    ...
    [self.urlRequest startRequest:tempDict pathUrl:@"/queryOrderListByTel"];
}
//订单状态：待付款-1，待发货-2，待收货-3，已完成-4，已取消-5
...
- (void)showVisibleCellImage {
    NSArray *visibleCellArray = [self.cancelOrderTable indexPathsForVisibleRows];
    for (NSIndexPath *indexPath in visibleCellArray) {
        CommodityOrderTableViewCell *cell = [self.cancelOrderTable cellForRowAtIndexPath:indexPath];
        NSArray *dataArray = self.orderMessage[indexPath.section][@"commodity"];
        OrderModel *data = dataArray[indexPath.row];
        NSArray *arr = [data.commodityImageName componentsSeparatedByString:@";"];
        [cell.commodityImage sd_setImageWithURL:arr[0]];
